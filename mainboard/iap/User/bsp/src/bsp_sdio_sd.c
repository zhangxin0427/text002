...
static u32 CSD_Tab[4],CID_Tab[4],RCA = 0;			//SD卡CSD(卡特定数据）、CID(卡识别号)、RCA(相对地址)数据
static u8 DeviceMode = SD_DMA_MODE;				//工作模式：DMA模式
static u8 StopCondition = 0;					//是否发送停止传输标志位，DMA多块读写的时候用到
volatile SD_Error TransferError = SD_OK;			//数据传输错误标记，DMA读写时使用
volatile u8 TransferEnd = 0;					//传输结束标记，DMA读写时使用
SD_CardInfo SDCardInfo;						//SD卡信息
__align(4) u8 SDIO_DATA_BUFFER[512];				//数据缓存区，确保4字节对齐
...
SD_Error SD_Init(void)
{
	...
	/*SDIO IO口初始化*/
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOF,ENABLE);
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;							//PA15上拉输入(SD_INSERT)	
